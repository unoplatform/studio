...
description: "`IFeed<T>` in MVUX: loading async data from a service or API into reactive data sources. Feeds automatically handle loading/error states. Use when transforming async data with operators like `Select` or `Where`, refreshing data using `Signal`, or understanding the difference between feeds and states. Create and use IFeed<T> for async data in MVUX."
metadata:
  author: uno-platform
  version: "2.3"
  category: mvux
