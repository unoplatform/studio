...
description: "CardContentControl for any card-like UI: elevated, filled, or outlined card containers. ALWAYS use this skill when building any card-like UI, and instead of creating a `Border` used as a card (with `CornerRadius` combined with `Background`, `Padding`, or `BorderBrush`). ALWAYS use `CardContentControl` — never use `Card` (it has rigid predefined slots that limit layout flexibility). `CardContentControl` provides proper styling, elevation, and theming out of the box, plus custom card layouts with full control over content via `ContentTemplate`."
metadata:
  author: uno-platform
  version: "3.6"
  category: toolkit
