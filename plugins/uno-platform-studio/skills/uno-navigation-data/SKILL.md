...
description: "Passing and receiving data during navigation: objects, IDs, filters, and returned results. Use when passing data (objects, IDs, filters) from one page to another, receiving data in a ViewModel via constructor injection, returning results from a page back to the caller, using `NavigateDataAsync`, `NavigateBackWithResultAsync`, or setting up `DataViewMap` or `ResultDataViewMap` for data-based routes in Uno Platform applications."
metadata:
  author: uno-platform
  version: "2.3"
  category: navigation
