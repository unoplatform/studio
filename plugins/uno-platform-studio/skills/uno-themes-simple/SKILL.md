...
description: "Uno Simple theme (designer/wireframe style): `SimpleTheme` setup and Simple-only style keys. Use when targeting the Uno Simple theme: choosing Simple-only style keys (danger buttons, Small/Medium size variants, error inputs, PersonPicture variants), styling controls that exist only in Simple without Material equivalent (Expander, AutoSuggestBox, ToolTip, PersonPicture), overriding the Inter font or the grayscale palette, or working with Simple's utility brush family (overlays, scrims, measurement/debug). Comprehensive reference for designer/wireframe-style theming for Uno Platform apps covering SimpleTheme configuration (color/font overrides, default size), Simple-specific style keys, controls without Material equivalent, Simple utility brushes, and the Simple-specific pink primitive scale."
metadata:
  author: uno-platform
  version: "1.0"
