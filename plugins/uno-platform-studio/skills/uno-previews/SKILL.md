...
- **Build in Debug to check a preview.** Release builds exclude the previews folder, so an error in a preview never shows there. One Debug build is enough: there is no need to force a rebuild, read the generated code, or build Release to confirm the folder is left out.
