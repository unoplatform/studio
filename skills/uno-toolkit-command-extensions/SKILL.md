...
description: "`CommandExtensions.Command` to invoke MVVM commands from controls without a built-in `Command`. Use when a UI event (Enter in `TextBox`, item click in `ListView`, `ToggleSwitch` toggle, `NavigationViewItem` invocation, any `UIElement` tap) must invoke an MVVM command without code-behind event handlers. The attached properties `CommandExtensions.Command` and `CommandExtensions.CommandParameter` bind these controls to MVVM commands."
metadata:
  author: uno-platform
  version: "2.5"
  category: toolkit
